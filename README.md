...
- J'ai utilisé mkcert (sous windows) pour généré les secrets (non push sur git), commandes faites :
 ...
 mkcert -install
 mkcert k8s-microproject.lan

 kubectl create secret tls k8s-microproject-tls \
  --cert=k8s-microproject.lan.pem \
  --key=k8s-microproject.lan-key.pem

  kubectl apply -f ingress.yaml
  minikube tunnel
  
  curl -k https://k8s-microproject.lan/
 ```
